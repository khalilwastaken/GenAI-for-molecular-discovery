...
class TrainLossDiscrete(nn.Module):
    ...
    def forward(self, masked_pred_X, masked_pred_E, pred_y, true_X, true_E, true_y, log: bool):
        ...
        # Remove masked rows (zeros) for calculation
        # Note: Be careful if 0 is a valid class index. Assuming 0-vector implies masking here.
        # Often DiGress uses the last channel or specific logic for masking, but this follows your provided file.
        ...
        loss_y = self.y_loss(pred_y, true_y) if pred_y.numel() > 0 and true_y.numel() > 0 else 0.0
        ...
        # Check if lambda_train has enough elements (usually [lambda_E, lambda_y])
    ...
    def log_epoch_metrics(self):
        ...
        # --- CORRECTION ICI ---
        # C'était self.train_y_loss (qui n'existe pas) -> self.y_loss
